...
if __name__ == "__main__":
    ...
    for frame in track(animation_frames, description="Creating GIF frames..."):
        ...
        try:
            temphead=Image.open(f'output/{name}/hd/internal/{int(frame[0])*2}-{int(frame[1])*2}.png')
        except:
            print(f'ERROR: [bright_red]A frame in your animation is selecting an invalid head ({int(frame[0])},{int(frame[1])} or for HD sprite sheets {int(frame[0])*2},{int(frame[1])*2}), or your heads are seperated unevenly in your sprite sheet. (Animation frame {framecount})')
            sys.exit()
